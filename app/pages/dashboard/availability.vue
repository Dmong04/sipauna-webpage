...
<template>
  <div class="m-4 **:transition-colors **:duration-200">
    ...
    <div class="flex flex-col lg:flex-row gap-4 lg:items-start mx-2">
      ...
      <!-- ── Filter panel ──────────────────────────────────────────────────── -->
      <aside
        class="w-full lg:w-64 shrink-0 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm p-5">
        ...
        <form @submit.prevent="search" class="flex flex-col gap-4">
          ...
          <div>
            ...
            <p v-if="filterDate && formattedDate" class="mt-1 text-xs text-gray-400 dark:text-gray-500 capitalize">{{
              formattedDate }}</p>
            <p v-if="isWeekend" class="mt-1 text-xs text-amber-600 dark:text-amber-400">Día de la semana domingo — sin clases
              regulares</p>
          </div>
        ...
        <div v-if="searched" class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 space-y-1.5">
          <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-2">Referencias
          </p>
      ...
      <div class="flex-1 min-w-0">
        ...
        <div v-if="!searched && !loading"
          ...
          <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Los resultados mostrarán qué aulas están libres en
            ese momento.</p>
        </div>
        ...
        <template v-else-if="searched">
          ...
          <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
            ...
            <!-- View toggle -->
            <div
              class="flex shrink-0 items-center rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-900 text-xs font-medium">
              <button @click="viewMode = 'list'"
                :class="['px-3 py-1.5 transition-colors', viewMode === 'list' ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400']">
                Lista
              </button>
              <button @click="viewMode = 'timeline'"
                :class="['px-3 py-1.5 transition-colors', viewMode === 'timeline' ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400']">
                Línea de tiempo
              </button>
            </div>
          </div>
          ...
          <div v-if="results.length === 0"
            class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex flex-col items-center py-16 text-gray-400">
          ...
          <div v-else-if="viewMode === 'list'" class="flex flex-col gap-3">
            <div v-for="room in results" :key="room.classroomId"
              class="rounded-xl border bg-white dark:bg-gray-900 shadow-sm overflow-hidden" :class="room.status === 'free' ? 'border-gray-200 dark:border-gray-700'
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p v-if="room.status === 'free'"
                      ...
                      <span v-if="room.busySlots.length" class="text-gray-400 dark:text-gray-500 ml-1">(tiene
                        ocupaciones fuera de tu horario)</span>
                    </p>
                    ...
                    <div v-if="room.status !== 'free' && room.busySlots.length" class="mt-2 flex flex-col gap-1.5">
                      ...
                      <div
                        v-for="(slot, i) in room.busySlots.filter(s => overlaps(filterStart, filterEnd, s.start, s.end))"
                        :key="i" class="flex items-start gap-2 text-xs rounded-lg border px-3 py-1.5"
                    ...
                    <div v-if="room.busySlots.filter(s => !overlaps(filterStart, filterEnd, s.start, s.end)).length"
                      class="mt-1.5">
                      <p class="text-xs text-gray-400 dark:text-gray-500">
                        +{{room.busySlots.filter(s => !overlaps(filterStart, filterEnd, s.start, s.end)).length}}
                        ocupación{{room.busySlots.filter(s => !overlaps(filterStart, filterEnd, s.start, s.end)).length
                        > 1 ? 'es' : '' }}
          ...
          <div v-else class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="min-w-140 bg-white dark:bg-gray-900 overflow-hidden">

              <!-- Hour header -->
              <div class="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60">
                <div
                  class="w-24 shrink-0 px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide border-r border-gray-200 dark:border-gray-700">
                  Aula
                </div>
                <div class="flex-1 relative" style="height: 32px;">
                  <div v-for="h in HOURS" :key="h" class="absolute top-0 flex flex-col items-center"
                ...
                <div
                  class="w-24 shrink-0 sticky right-0 border-l border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60">
                </div>
              </div>

              <!-- Rows -->
              <div v-for="(room, ri) in results" :key="room.classroomId"
                class="group flex hover:bg-gray-50 dark:hover:bg-white/2"
                :class="{ 'border-t border-gray-100 dark:border-gray-800': ri > 0 }">

                <!-- Classroom label -->
                <div
                  class="w-24 shrink-0 px-3 py-3 border-r border-gray-100 dark:border-gray-800 flex flex-col justify-center">
                ...
                <div class="flex-1 relative" style="height: 48px;">
                  ...
                  <div v-for="h in HOURS" :key="h" class="absolute top-0 bottom-0 w-px bg-gray-100 dark:bg-gray-800"
                    :style="{ left: `${(h * 60 - TIMELINE_START) / TIMELINE_SPAN * 100}%` }">
                  </div>
                ...
                <div
                  class="w-24 shrink-0 sticky right-0 border-l border-gray-100 dark:border-gray-800 flex items-center justify-center px-2 bg-white dark:bg-gray-900 group-hover:bg-gray-50 dark:group-hover:bg-white/2 transition-colors duration-100">
                  <NuxtLink v-if="room.status === 'free'" :to="bookingLink(room.code)"
