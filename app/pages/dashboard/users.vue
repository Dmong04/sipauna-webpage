...
<template>
  <section class="m-4 **:transition-colors **:duration-300">
    ...
    <div class="mx-2 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
      ...
      <div v-else-if="users.length === 0" class="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          <tr v-for="user in users" :key="user.userId"
            ...
            <td class="px-4 py-3">
              ...
              <span v-else :class="['text-xs font-medium px-2 py-0.5 rounded-full', ROLE_BADGE[user.roleName] ?? ROLE_BADGE.invitado]">
                {{ user.roleName }}
              </span>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center justify-end gap-2">
                <button v-if="editingId !== user.userId" @click="startEdit(user)"
                  title="Cambiar rol"
    ...
    <p class="mx-2 mt-2 text-xs text-gray-400 dark:text-gray-600">
